  ...
  const fetchData = async () => {
    try {
      ...
      if (err.response?.status === 401 || err.response?.status === 403) {
        ...
        // Profile auto-created backend, but maybe bookings fail
        setBookings([]);
        setError(null); // Backend now handles, clear error
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-8">Please update your profile to continue.</p>
          <div className="space-y-3">
            <button 
              onClick={() => navigate('/search')}
              className="w-full px-6 py-3 bg-indigo-600 text-white rounded-2xl hover:bg-indigo-700"
            >
              ← Back to Search
            </button>
            <button 
              onClick={handleLogout}
              className="w-full px-6 py-3 bg-gray-600 text-white rounded-2xl hover:bg-gray-700"
            >
              Logout
            </button>
          </div>
